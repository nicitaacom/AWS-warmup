...
const sendTelegramErrorMessage = async (errorMessage:string,errorStackTrace:string) => {
...
const sendEmailError = async (errorMessage:string,errorStackTrace:string) => {
  try {
    if (!process.env.UPSTASH_REDIS_URL) throw Error("It's no UPSTASH_REDIS_URL")
    if (!process.env.SEND_EMAILS_TO) throw Error("It's no SEND_EMAILS_TO")
    if (!process.env.DOMAIN) throw Error("It's no DOMAIN")
    ...
    if (!encryptedResend) throw Error("No encryptedResend returned from redis - contact support")

    const decryptedEnvResend = await decryptResend(encryptedResend);
    if (typeof decryptedEnvResend === 'string') throw Error(`Error decrypting resend - contact support (recreate email account first)
    error message:${decryptedEnvResend}`)
    const resend = new Resend(decryptedEnvResend.value); // Resend defined outside VM
    ...
    if (resend_error) throw Error(resend_error.message)
  } catch (error:unknown) {
...
const sendDiscordErrorMessage = async (errorMessage:string,errorStackTrace:string) => {
  ...
  try {
    if (!process.env.DISCORD_WEBHOOK_URL) throw Error("It's no DISCORD_WEBHOOK_URL")
...
const sendTwilioSMSError = async (errorMessage:string,errorStackTrace:string) => {
  try {
...
export const sendErrorNotifications = async (errorMessage:string,errorStackTrace:string) => {
...
export function renderErrorEmailString(emailBody:string,errorStackTrace:string) {
  const now = new Date()
...
export async function decryptResend(encryptedResendEnvValue:string) {
  try {
